...
RRTStar::RRTStar(pair<double,double> start, pair<double,double> goal, int width, int height, float search_r, int board_size, int max_try, double max_dist, double goal_sample_rate, vector<Rect> boundary,vector<Rect> obs_rectangle, vector<Point3f> obs_circle)
    : start(start, start, 0), goal(goal, goal, 0), width(width), height(height), search_r(search_r), board_size(board_size), max_try(max_try),
...
vector<pair<double,double>> RRTStar::plan(double& cost) 
{
...
RRTStar::Node RRTStar::get_random_node() {
    random_device rd;
    mt19937 gen(rd());
...
RRTStar::Node RRTStar::get_nearest_neighbor(map<pair<double,double>, Node, less<>>& node_list, Node node) {
...
RRTStar::Node RRTStar::get_new_node(Node node_rand, Node node_near) {
    ...
    if (isCollision(&node_new, &node_near)) {
        ...
        {
            double new_dist = distance(node.second, node_new);
            if (new_dist < search_r && !isCollision(&node_new, &node.second))
            {
                double cost = node.second.cost + new_dist;
                if (node_new.cost > cost)
                {
                    node_new.parent = node.second.position;
                    node_new.cost = cost;
                }
                else
                {
                    cost = node_new.cost + new_dist;
                    if (node.second.cost > cost)
                    {
                        node.second.parent = node_new.position;
                        node.second.cost = cost;
                    }
                }
            }
            else
...
bool RRTStar::isCollision(Node* node1, Node* node2) {
...
bool RRTStar::isInBoundary(Node* node)
{
...
bool RRTStar::isInObstacles(Node* node) {
...
float RRTStar::cross(const pair<double,double>& p1, const pair<double,double>& p2, const pair<double,double>& p3) {
...
bool RRTStar::isInterRect(Node* node1, Node* node2, const cv::Rect& rect) {
...
bool RRTStar::isInterCircle(Node* node1, Node* node2, Point3f circle) {
...
vector<pair<double,double>> RRTStar::extractPath(map<pair<double,double>, Node, less<>>& closed_list) 
{
    vector<pair<double,double>> path;
    Node node(goal);
...
void RRTStar::visualize(double cost, vector<pair<double,double>>& path) 
{
    int cell_size = 20;

    VideoWriter video("RRTStar_animation.mp4", VideoWriter::fourcc('m', 'p', '4', 'v'), 100, Size(width*cell_size, height*cell_size));

    Mat img = Mat::zeros(height*cell_size, width*cell_size, CV_8UC3);
    img.setTo(Scalar(255, 255, 255));

    // Title and Cost
    std::string title = "RRTStar";
    ...
    video.release();
    // Show the image
    imwrite("RRTStar.png", img);
    imshow("RRTStar Path", img);
    waitKey(0);
    destroyAllWindows();
}